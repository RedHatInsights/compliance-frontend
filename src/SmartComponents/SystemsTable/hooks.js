...
export const useOnSelect = (onSelectProp, items, preselectedSystems, total) => {
  ...
  const isPageSelected =
    items.filter((item) => selectedSystemIds.includes(item.id)).length ===
    items.length;

  const onSelectCallback = (selected) => onSelectProp && onSelectProp(selected);

  const onSelect = (_event, select, _index, row) => {
    const system = items.find(({ id }) => id === row.id);
    if (!system) {
      console.error(
        `System identifed as ${row.id} not found in items for selection!`
      );
      return;
    }
  ...
  const onBulkSelect = () => {
    const selectedSystemsWithoutItems = (selectedSystems || []).filter(
      (system) => !items.map((item) => item.id).includes(system.id)
    );
    const notAllSelected = selectedSystems.length <= total;
    const selected = notAllSelected
      ? !isPageSelected
        ? [...selectedSystemsWithoutItems, ...items]
        : selectedSystemsWithoutItems
      : [];
...
export const useSystemsExport = ({
  ...
  const fetchSystems = useFetchSystems({
    query,
    variables: {
      filter: selectionFilter ? `${filter} and (${selectionFilter})` : filter,
      ...(policyId && { policyId }),
    },
  });

...
const useBulkSelect = ({
  ...
  const items = (sorter ? sorter(propItems) : propItems).map(
    selectItemTransformer
  );
  ...
  const allFiltertedSelected =
    selectedIds?.length > 0
      ? allItemsIncluded(itemIds(filteredItems), selectedIds)
      : false;
  ...
  const selectFiltered = () =>
    onSelectCallback(() => {
      ...
      return allFiltertedSelected
        ? unselectItems(currentFilteredIds)
        : selectItems(currentFilteredIds);
    });

  const selectAll = () => onSelectCallback(() => itemIds(items));

  const selectFilteredOrAll = () => (filtered ? selectFiltered() : selectAll());
        ...
        toolbarProps: {
          bulkSelect: {
            ...
            items: [
              ...
              {
                title: `${selectOrUnselect(
                  filtered ? allFiltertedSelected : allSelected
                )} all (${allCount} items)`,

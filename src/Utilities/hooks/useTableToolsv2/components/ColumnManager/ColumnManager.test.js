...
describe('ColumnManager', () => {
  ...
  it('expect to render without error and all columns', () => {
    expect(
      renderJson(<ColumnManager columns={columns} selectProp="title" />)
    ).toMatchSnapshot();
  });
  ...
  it('expect to render with custom title', () => {
    expect(
      renderJson(<ColumnManager columns={columns} title="TEST TITLE" />)
    ).toMatchSnapshot();
  });

  it('expect to render with all column checkboxes disabled', () => {
    const allColumnsRequired = columns.map((column) => ({
      ...column,
      isRequired: true,
    }));
    expect(
      renderJson(<ColumnManager columns={allColumnsRequired} />)
    ).toMatchSnapshot();
  });
});

...
const useFilterConfig = (initialConfig, arrayToFilter) => {
  ...
  const [activeFilters, setActiveFilters] = useState(
    filterConfigBuilder.initialDefaultState()
  );
  ...
  const filteredArray = arrayToFilter
    ? filterConfigBuilder.applyFilterToObjectArray(arrayToFilter, activeFilters)
    : undefined;
  ...
  return {
    ...
    activeFilters,
    activeFilterValues,
    filterString: !arrayToFilter
      ? filterConfigBuilder.getFilterBuilder().buildFilterString(activeFilters)
      : '',
  };
};

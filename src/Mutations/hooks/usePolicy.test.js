...
describe('usePolicy', () => {
    afterEach(() => {
        [
            useCreateBusinessObjective, usePolicyMutation, useAssociateSystems,
            useAssociateSystems
    ...
    it('creates new policy with no system and rule associations', async () => {
        ...
        let progressCount = 0;
        const onProgress = () => { progressCount++; };

        const createBusinessObjective = jest.fn(async () => (null));
        const policyMutation = jest.fn(async () => (
            { id: '1' }
        ));
    ...
    it('creates new policy with systems, tailoring, and business objective', async () => {
        ...
        const profile1SelectedRuleRefIds = {
            id: '1', ruleRefIds: ['ref1', 'ref2']
        };
        const profile2SelectedRuleRefIds = {
            id: '2', ruleRefIds: ['ref3', 'ref4']
        };
        const selectedRuleRefIds = [profile1SelectedRuleRefIds, profile2SelectedRuleRefIds];
        ...
        expect(associateRules).toBeCalledWith(profile1SelectedRuleRefIds, newPolicyProfiles);
        expect(associateRules).toBeCalledWith(profile2SelectedRuleRefIds, newPolicyProfiles);
    ...
    it('updates policy, its systems, tailoring, and business objective', async () => {
        ...
        expect(createBusinessObjective).toBeCalledWith(oldPolicy, businessObjective);
